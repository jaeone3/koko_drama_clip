...
OUT = ROOT / "outputs/final_15_with_cta_black_side_banner"
FINAL = OUT / "final_15_original_only_with_cta_black_side_banner.mp4"
...
def main():
    ...
    manifest = {
        ...
        "cta_screen_height_ratio": BANNER_HEIGHT / 1920,
        "layout": "full-width black banner background with uncropped 9:16 CTA centered",
    }
    (OUT / "manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
    print(FINAL)
    print(f"duration={manifest['duration']:.3f}")
    print(f"cta_start={AD_START:.3f}")
    print(f"banner_size={BANNER_WIDTH}x{BANNER_HEIGHT}")
